...
class Invoice:
    ...
    def create_invoice(customer_id, items, discount=0.0, tax=0.0):
        ...
        cursor.execute("""
            ...
            VALUES (?, ?, ?, ?, ?)
        """, (customer_id, invoice_date, discount, tax, total_after_tax))
    ...
    def update_invoice(invoice_id, customer_id, items, discount=0.0, tax=0.0):
        ...
        for product_id, quantity in old_items:
            product = Product.get_product_by_id(product_id)
            if product:
                Product.update_stock(product_id, product.stock_quantity + quantity)
        ...
        cursor.execute("""
            ...
            WHERE invoice_id = ?
        """, (customer_id, discount, tax, total_after_tax, invoice_id))
    ...
    def print_receipt(invoice_id):
        ...
        for item in invoice['items']:
            print(f" Product: {item['product_name']} | Quantity: {item['quantity']} | Unit Price: {item['unit_price']}")
        print("-----------------------------")
